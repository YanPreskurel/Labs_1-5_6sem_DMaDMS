...
create user C##Lab4 identified by "qwerty12";
GRANT ALL PRIVILEGES TO C##Lab4;
...
CREATE OR REPLACE PACKAGE json_parser AS
    ...
    FUNCTION parse_name_section(json_query_string JSON_ARRAY_T, search_name VARCHAR2,
        ...
        RETURN VARCHAR2;
    FUNCTION
        parse_table_section(json_query_string JSON_ARRAY_T) RETURN VARCHAR2;
    FUNCTION
        parse_scalar_array(json_query_string JSON_ARRAY_T) RETURN VARCHAR2;
    FUNCTION
        parse_select(json_query_string JSON_OBJECT_T) RETURN VARCHAR2;
    FUNCTION
        condition_checker(json_query_string JSON_OBJECT_T,
                          is_in_join_segment BOOLEAN DEFAULT FALSE)
        RETURN VARCHAR2;
    FUNCTION
        parse_where_section(json_query_string JSON_OBJECT_T) RETURN VARCHAR2;
    FUNCTION
        parse_compound_conditions(json_query_string JSON_ARRAY_T,
                                  join_condition VARCHAR2,
                                  is_in_join_segment BOOLEAN DEFAULT FALSE)
        RETURN VARCHAR2;
    FUNCTION
        parse_between_condition(json_values JSON_OBJECT_T) RETURN VARCHAR2;
    FUNCTION
        parse_join_section(json_query_string JSON_ARRAY_T) RETURN VARCHAR2;
    --DML section
    FUNCTION
        parse_values_as_array(json_columns JSON_ARRAY_T,
                              json_values JSON_ARRAY_T,
                              new_val_type NUMBER) RETURN VARCHAR2;
    FUNCTION
        parse_values_as_select(json_columns JSON_ARRAY_T,
                               json_values JSON_OBJECT_T,
                               new_val_type NUMBER) RETURN VARCHAR2;
    FUNCTION
        parse_new_values_section(json_columns JSON_ARRAY_T,
                                 json_values JSON_ELEMENT_T,
                                 new_val_type NUMBER) RETURN VARCHAR2;
    FUNCTION
        parse_dml_section(json_query_string JSON_OBJECT_T) RETURN VARCHAR2;
    FUNCTION
        parse_JSON_to_SQL(json_query_string JSON_OBJECT_T) RETURN VARCHAR2;
END json_parser;
...
CREATE OR REPLACE PACKAGE BODY json_parser AS
    ...
    FUNCTION
        parse_table_section(json_query_string JSON_ARRAY_T)
        RETURN VARCHAR2
        IS
    ...
    FUNCTION
        parse_simple_condition(json_query_string JSON_OBJECT_T,
                               is_in_join_segment BOOLEAN DEFAULT FALSE)
        RETURN VARCHAR2
        IS
    ...
    BEGIN
        res_str
            := res_tab_name || json_query_string.get_string('col') || ' ';

        IF
            is_in_join_segment AND json_query_string.get_string('comparator') != '=' THEN
            RAISE_APPLICATION_ERROR(-20009, 'Unexpected value ('
                || json_query_string.get_string('comparator') || ') expected (=) in "join section"');
        END IF;

        res_str
            := res_str || UPPER(json_query_string.get_string('comparator')) || ' ';
        buff_json_element
            := json_query_string.get('value');

        IF
            is_in_join_segment THEN
            res_str := res_str || json_query_string.get_string('value');
        ELSIF
            buff_json_element.is_scalar THEN
            res_str := res_str || buff_json_element.to_string;
        ELSIF
            buff_json_element.is_array THEN
    ...
    BEGIN
        FOR i IN 0..json_query_string.get_size - 1
            LOOP
                ...
                res_str
                    := res_str || condition_checker(buff_json_object, is_in_join_segment)
                    || ' ' || join_condition || ' ';
            END LOOP;
    ...
    FUNCTION
        parse_between_condition(json_values JSON_OBJECT_T)
        RETURN VARCHAR2
        IS
    BEGIN
        RETURN '(' || json_values.get_String('col') || ' BETWEEN ' || json_values.get_Number('min') || ' AND ' ||
               json_values.get_Number('max') || ')';
    ...
    BEGIN
        FOR i IN 0..json_query_string.get_size - 1
            LOOP
                ...
                IF
                    buff_json_object.has('select') THEN
                    res_str := res_str || CHR(10) || '(' || CHR(10)
                        || parse_select(buff_json_object.get_object('select')) || ')';
                ELSIF
                    buff_json_object.has('table') THEN
    ...
    FUNCTION
        parse_group_by_section(json_query_string JSON_ARRAY_T) RETURN VARCHAR2
        IS
    BEGIN
        RETURN NULL;
    ...
    BEGIN
        IF
            NOT json_query_string.has('what') THEN
        ...
        IF
            NOT json_query_string.has('from') THEN
    ...
    BEGIN
        IF
            json_columns.get_size != json_values.get_size THEN
    ...
    BEGIN
        IF
            NOT json_values.has('select') THEN
    ...
    --DDL
    FUNCTION
        get_full_cons_name(short_cons_name VARCHAR2) RETURN VARCHAR2
        IS
    BEGIN
    ...
    FUNCTION
        parse_check_constraint(json_columns JSON_ARRAY_T,
    ...
    BEGIN
        buff_json_array
            := json_reference.get_array('columns');
        IF
            buff_json_array.get_size != json_columns.get_size THEN
    ...
    FUNCTION
        parse_outline_constraints_section(json_constraints JSON_ARRAY_T)
        RETURN VARCHAR2
        IS
        res_str VARCHAR2(500);
        buff_str
    ...
    BEGIN
        FOR i IN 0..json_constraints.get_size - 1
            LOOP
                ...
                res_str
                    := res_str || 'CONSTRAINT ' || buff_json_object.get_string('cons_name') || ' ';
                buff_str
                    := get_full_cons_name(buff_json_object.get_string('cons_type'));

                IF
                    buff_str IS NULL THEN
    ...
    FUNCTION
        create_auto_increment_trigger(tab_name VARCHAR2, col_name VARCHAR2)
        RETURN VARCHAR2
        IS
    ...
    BEGIN
        seq_str
            := 'CREATE SEQUENCE ' || tab_name || '_' || col_name || '_seq START WITH 1;';
        trigger_str
            := 'CREATE OR REPLACE TRIGGER ' || tab_name || '_' || col_name || '_auto'
    ...
    FUNCTION
        parse_create_section(tab_name VARCHAR2,
    ...
    BEGIN
        FOR i IN 0..json_columns.get_size - 1
            LOOP
                ...
                res_str
                    := res_str || buff_json_object.get_string('col_name') || ' '
                    || buff_json_object.get_string('data_type') || ' ';
                IF buff_json_object.has('inline_constraints') THEN
                    ...
                    FOR j IN 0..buff_json_array.get_size - 1
                        LOOP
                            ...
                            THEN
                                trig_str :=
                                        create_auto_increment_trigger(tab_name,
                                                                      buff_json_object.get_string('col_name'));
                            END IF;
                            res_str
                                := res_str || buff_json_array.get_string(j) || ' ';
                        END LOOP;
                end if;

                res_str
                    := RTRIM(res_str, ' ') || ',' || CHR(10);
            END LOOP;
        IF
            json_constraints IS NOT NULL THEN
    ...
    FUNCTION
        parse_ddl_section(json_query_string JSON_OBJECT_T) RETURN VARCHAR2
        IS
    ...
    BEGIN
        IF
            NOT json_query_string.has('type') THEN
        ...
        res_str
            := UPPER(json_query_string.get_string('type')) || ' TABLE '
            || json_query_string.get_string('tab_name');


        IF
            json_query_string.has('outline_constraints') THEN
            buff_json_array := json_query_string.get_array('outline_constraints');
        END IF;

        IF
            res_str LIKE 'DROP%' THEN
            RETURN res_str;
        ELSIF
            res_str LIKE 'CREATE%' THEN
...
BEGIN
    ...
    (json_parser.parse_JSON_to_SQL(JSON_OBJECT_T(json_content)));
END;
