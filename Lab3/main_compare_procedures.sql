...
AUTHID CURRENT_USER
AS
    v_prod_name VARCHAR2(100) := UPPER(prod);
    v_dev_name VARCHAR2(100) := UPPER(dev);
    v_script VARCHAR2(4000);
    v_count NUMBER;
BEGIN
    -- Выводим процедуры, которые есть в dev схеме, но нет в prod схеме
    FOR proc IN (SELECT object_name
                 FROM all_procedures
                 WHERE owner = v_dev_name AND object_type = 'PROCEDURE'
                 MINUS
                 SELECT object_name
                 FROM all_procedures
                 WHERE owner = v_prod_name AND object_type = 'PROCEDURE')
    LOOP
        DBMS_OUTPUT.PUT_LINE('No dev procedure "' || proc.object_name || '" in prod schema');
    END LOOP;

    -- Сравниваем процедуры, которые есть в dev схеме и в prod схеме
    FOR dev_proc IN (SELECT object_name, dbms_metadata.get_ddl('PROCEDURE', object_name, v_dev_name) AS proc_text
                     FROM all_objects
                     WHERE object_type = 'PROCEDURE' AND owner = v_dev_name)
    LOOP
        -- Проверяем, существует ли процедура в prod схеме
        BEGIN
            SELECT COUNT(*)
            INTO v_count
            FROM all_objects
            WHERE object_type = 'PROCEDURE' AND object_name = dev_proc.object_name AND owner = v_prod_name;
            
            IF v_count = 0 THEN
                -- Если процедура отсутствует в prod схеме, выводим её DDL
                v_script := dev_proc.proc_text;
                v_script := REPLACE(v_script, v_dev_name, v_prod_name);
                DBMS_OUTPUT.PUT_LINE(v_script);
            ELSE
                -- Если процедура существует в обеих схемах, сравниваем их DDL
                DECLARE
                    v_prod_ddl VARCHAR2(4000);
                    v_dev_ddl VARCHAR2(4000);
                BEGIN
                    SELECT dbms_metadata.get_ddl('PROCEDURE', dev_proc.object_name, v_prod_name)
                    INTO v_prod_ddl
                    FROM dual;
                    
                    v_dev_ddl := dev_proc.proc_text;
                    
                    IF v_prod_ddl != v_dev_ddl THEN
                        -- Если DDL процедур отличается, выводим сообщение
                        DBMS_OUTPUT.PUT_LINE('Difference found in procedure ' || dev_proc.object_name);
                        DBMS_OUTPUT.PUT_LINE('Prod DDL:');
                        DBMS_OUTPUT.PUT_LINE(v_prod_ddl);
                        DBMS_OUTPUT.PUT_LINE('Dev DDL:');
                        DBMS_OUTPUT.PUT_LINE(v_dev_ddl);
                    END IF;
                END;
            END IF;
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                NULL; -- Процедура не найдена, переходим к следующей
        END;
    END LOOP;

    -- Удаляем процедуры, которые есть в prod схеме, но отсутствуют в dev схеме
    FOR prod_proc IN (SELECT object_name
                      FROM all_objects
                      WHERE object_type = 'PROCEDURE' AND owner = v_prod_name)
    LOOP
        -- Проверяем, существует ли процедура в dev схеме
        BEGIN
            SELECT COUNT(*)
            INTO v_count
            FROM all_objects
            WHERE object_type = 'PROCEDURE' AND object_name = prod_proc.object_name AND owner = v_dev_name;
            
            IF v_count = 0 THEN
                DBMS_OUTPUT.PUT_LINE('DROP PROCEDURE ' || v_prod_name || '.' || prod_proc.object_name || ';');
            END IF;
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                NULL; -- Процедура не найдена, переходим к следующей
...
create or replace procedure cmp_tbl (
    prod in varchar2,
    dev in varchar2
) authid current_user is
    v_dev_table_name all_tables.table_name%type;
...
begin
    ...
    select count(*) into v_count_circular from (
        with table_hierarchy as (
            ...
            from (
                ...
                from all_constraints where constraint_type = 'R' and owner = dev_name
            ) join (
                select owner parent_owner, constraint_name, table_name parent_table
                from all_constraints where constraint_type = 'P' and owner = dev_name
            ) using (parent_owner, constraint_name)
        )
        select distinct child_owner, child_table
        from (
    ...
    for i in reverse 1..v_table_list.count loop
        ...
        if v_table_count = 0 then
            ...
            -- script to remove extra columns
            for prod_col_rec in (select column_name, data_type, data_length, data_precision, data_scale
                                from all_tab_cols where owner = prod_name and table_name = v_dev_table_name and column_name not like 'SYS%') loop
                select count(*) into v_table_count from all_tab_cols where owner = dev_name and table_name = v_dev_table_name and column_name = prod_col_rec.column_name;
